...
#define WPAN_SETUP_RETRY	3

struct vlist_tree wpan_devices;
struct avl_tree wpan_drivers;
...
static void
prepare_config(struct wpan_device *wdev, struct blob_buf *buf, bool up)
{
	struct wpan_interface *vif;
	void *l, *i;
...
static bool
wpan_process_check(struct wpan_process *proc)
{
	return check_pid_path(proc->pid, proc->exe);
}

static void
wpan_complete_kill_request(struct wpan_device *wdev)
{
...
static void
wpan_process_free(struct wpan_device *wdev, struct wpan_process *proc)
{
	D(WIRELESS, "WPAN device '%s' free pid %d\n", wdev->name, proc->pid);
	list_del(&proc->list);
	free(proc);

	if (list_empty(&wdev->script_proc))
		wpan_complete_kill_request(wdev);
}

static void
wpan_close_script_proc_fd(struct wpan_device *wdev)
{
...
static void
wpan_process_kill_all(struct wpan_device *wdev, int signal, bool free)
{
	struct wpan_process *proc, *tmp;

	list_for_each_entry_safe(proc, tmp, &wdev->script_proc, list) {
		bool check = wpan_process_check(proc);

		if (check) {
			D(WIRELESS, "WPAN device '%s' kill pid %d\n", wdev->name, proc->pid);
			kill(proc->pid, signal);
		}

		if (free || !check)
			wpan_process_free(wdev, proc);
	}

	if (free)
		wpan_close_script_proc_fd(wdev);
}

static void
wpan_device_free_state(struct wpan_device *wdev)
{
	struct wpan_interface *vif;
...
static void wpan_interface_handle_link(struct wpan_interface *vif, bool up)
{
	struct interface *iface;
	struct blob_attr *cur;
	const char *network;
	...
	if (up) {
		...
		if (dev) {
			dev->wpan_isolate = vif->isolate;
			dev->wpan = true;
			dev->wpan_ap = vif->ap_mode;
		}
	}
...
static void
wpan_device_setup_cancel(struct wpan_device *wdev)
{
	if (wdev->cancel)
		return;

	D(WIRELESS, "Cancel WPAN device '%s' setup\n", wdev->name);
	wdev->cancel = true;
	uloop_timeout_set(&wdev->timeout, 10 * 1000);
}

static void
wpan_device_run_handler(struct wpan_device *wdev, bool up)
{
	...
	D(WIRELESS, "WPAN device '%s' run %s handler\n", wdev->name, action);
...
static void
__wpan_device_set_up(struct wpan_device *wdev)
{
	if (wdev->disabled)
		return;
	...
	wpan_device_run_handler(wdev, true);
}

static void
wpan_device_free(struct wpan_device *wdev)
{
	vlist_flush_all(&wdev->interfaces);
	avl_delete(&wpan_devices.avl, &wdev->node.avl);
	free(wdev->config);
	free(wdev->prev_config);
	free(wdev);
}
...
static void
wdev_handle_config_change(struct wpan_device *wdev)
{
	...
	case IFC_RELOAD:
		...
		if (wdev->autostart)
			__wpan_device_set_up(wdev);
		break;
	case IFC_REMOVE:
		wpan_device_free(wdev);
		break;
	}
}

static void
wpan_device_mark_down(struct wpan_device *wdev)
{
	struct wpan_interface *vif;
	...
	vlist_for_each_element(&wdev->interfaces, vif, node)
		wpan_interface_handle_link(vif, false);

	wpan_process_kill_all(wdev, SIGTERM, true);

	wdev->cancel = false;
	wdev->state = IFS_DOWN;
	wpan_device_free_state(wdev);
	wdev_handle_config_change(wdev);
}

static void
wpan_device_setup_timeout(struct uloop_timeout *timeout)
{
	struct wpan_device *wdev = container_of(timeout, struct wpan_device, timeout);

	netifd_kill_process(&wdev->script_task);
	wdev->script_task.cb(&wdev->script_task, -1);
	wpan_device_mark_down(wdev);
}

void
wpan_device_set_up(struct wpan_device *wdev)
{
	wdev->retry = WPAN_SETUP_RETRY;
	wdev->autostart = true;
	__wpan_device_set_up(wdev);
}

static void
__wpan_device_set_down(struct wpan_device *wdev)
{
	if (wdev->state == IFS_TEARDOWN || wdev->state == IFS_DOWN)
		return;

	if (wdev->script_task.uloop.pending) {
		wpan_device_setup_cancel(wdev);
		return;
	}

	wdev->state = IFS_TEARDOWN;
	wpan_device_run_handler(wdev, false);
}

static void
wpan_device_mark_up(struct wpan_device *wdev)
{
	struct wpan_interface *vif;

	if (wdev->cancel) {
		wdev->cancel = false;
		__wpan_device_set_down(wdev);
		return;
	}
	...
	wdev->state = IFS_UP;
	vlist_for_each_element(&wdev->interfaces, vif, node)
		wpan_interface_handle_link(vif, true);
}

static void
wpan_device_retry_setup(struct wpan_device *wdev)
{
...
static void
wpan_device_script_task_cb(struct netifd_process *proc, int ret)
{
	struct wpan_device *wdev = container_of(proc, struct wpan_device, script_task);

	switch (wdev->state) {
	case IFS_SETUP:
		wpan_device_retry_setup(wdev);
		break;
	case IFS_TEARDOWN:
		wpan_device_mark_down(wdev);
		break;
...
void
wpan_device_set_down(struct wpan_device *wdev)
{
	wdev->autostart = false;
	__wpan_device_set_down(wdev);
}
...
static void
wdev_set_config_state(struct wpan_device *wdev, enum interface_config_state s)
{
...
static void
wdev_prepare_prev_config(struct wpan_device *wdev)
{
	if (wdev->prev_config)
		return;
...
static void
wdev_change_config(struct wpan_device *wdev, struct wpan_device *wd_new)
{
	...
	D(WIRELESS, "Update configuration of wpan device '%s'\n", wdev->name);
	free(wdev->config);
...
static void
wdev_create(struct wpan_device *wdev)
{
	wdev->retry = WPAN_SETUP_RETRY;
...
{
	struct wpan_device *wd_old = container_of(node_old, struct wpan_device, node);
	struct wpan_device *wd_new = container_of(node_new, struct wpan_device, node);

	if (wd_old && wd_new) {
		wdev_change_config(wd_old, wd_new);
	} else if (wd_old) {
		D(WIRELESS, "Delete wpan device '%s'\n", wd_old->name);
		wdev_set_config_state(wd_old, IFC_REMOVE);
	} else if (wd_new) {
		D(WIRELESS, "Create wpan device '%s'\n", wd_new->name);
		wdev_create(wd_new);
	}
}

static void
wpan_add_handler(const char *script, const char *name, json_object *obj)
{
	...
	avl_insert(&wpan_drivers, &drv->node);
	D(WIRELESS, "Add handler for script %s: %s\n", script, name);
}

void wpan_init(void)
{
	vlist_init(&wpan_devices, avl_strcmp, wdev_update);
	wpan_devices.keep_old = true;
	wpan_devices.no_delete = true;

	avl_init(&wpan_drivers, avl_strcmp, false, NULL);
	drv_fd = netifd_open_subdir("wpan");
	if (drv_fd < 0)
		return;

	netifd_init_script_handlers(drv_fd, wpan_add_handler);
}

static void
wpan_interface_init_config(struct wpan_interface *vif)
{
...
vif_update(struct vlist_tree *tree, struct vlist_node *node_new,
	   struct vlist_node *node_old)
{
	struct wpan_interface *vif_old = container_of(node_old, struct wpan_interface, node);
	struct wpan_interface *vif_new = container_of(node_new, struct wpan_interface, node);
	...
	if (vif_old && vif_new) {
		...
		D(WIRELESS, "Update wpan interface %s on device %s\n", vif_new->name, wdev->name);
		wpan_interface_handle_link(vif_old, false);
		free(vif_old->config);
		vif_old->config = blob_memdup(vif_new->config);
		vif_old->isolate = vif_new->isolate;
		vif_old->ap_mode = vif_new->ap_mode;
		wpan_interface_init_config(vif_old);
		free(vif_new);
	} else if (vif_new) {
		D(WIRELESS, "Create new wpan interface %s on device %s\n", vif_new->name, wdev->name);
		vif_new->section = strdup(vif_new->section);
		vif_new->config = blob_memdup(vif_new->config);
		wpan_interface_init_config(vif_new);
	} else if (vif_old) {
		D(WIRELESS, "Delete wpan interface %s on device %s\n", vif_old->name, wdev->name);
		free((void *) vif_old->section);
		free(vif_old->config);
		free(vif_old);
	}
...
static void
wpan_proc_poll_fd(struct uloop_fd *fd, unsigned int events)
{
	struct wpan_device *wdev = container_of(fd, struct wpan_device, script_proc_fd);
	char buf[128];
...
static void
wpan_device_check_script_tasks(struct uloop_timeout *timeout)
{
	struct wpan_device *wdev = container_of(timeout, struct wpan_device, script_check);
	struct wpan_process *proc, *tmp;
	bool restart = false;

	list_for_each_entry_safe(proc, tmp, &wdev->script_proc, list) {
		if (wpan_process_check(proc))
			continue;
...
void
wpan_device_create(struct wpan_driver *drv, const char *name, struct blob_attr *data)
{
	struct wpan_device *wdev;
	char *name_buf;
	...
	wdev->timeout.cb = wpan_device_setup_timeout;
	wdev->script_task.cb = wpan_device_script_task_cb;
	wdev->script_task.dir_fd = drv_fd;
	wdev->script_task.log_prefix = wdev->name;

	wdev->script_proc_fd.fd = -1;
	wdev->script_proc_fd.cb = wpan_proc_poll_fd;

	wdev->script_check.cb = wpan_device_check_script_tasks;

	vlist_add(&wpan_devices, &wdev->node, wdev->name);
}
...
static void
wpan_interface_status(struct wpan_interface *iface, struct blob_buf *b)
{
	void *i;
...
void
wpan_device_status(struct wpan_device *wdev, struct blob_buf *b)
{
	struct wpan_interface *iface;
	void *c, *i;
	...
	vlist_for_each_element(&wdev->interfaces, iface, node)
		wpan_interface_status(iface, b);
	blobmsg_close_array(b, i);
	blobmsg_close_table(b, c);
}

void
wpan_device_get_validate(struct wpan_device *wdev, struct blob_buf *b)
{
...
static void
wpan_interface_set_data(struct wpan_interface *vif)
{
	enum {
		VIF_DATA_IFNAME,
		__VIF_DATA_MAX,
...
static int
wpan_device_add_process(struct wpan_device *wdev, struct blob_attr *data)
{
	enum {
		PROC_ATTR_PID,
		PROC_ATTR_EXE,
...
static int
wpan_device_process_kill_all(struct wpan_device *wdev, struct blob_attr *data,
				 struct ubus_request_data *req)
{
	...
	wpan_process_kill_all(wdev, signal, immediate);
...
static int
wpan_device_set_retry(struct wpan_device *wdev, struct blob_attr *data)
{
...
int
wpan_device_notify(struct wpan_device *wdev, struct blob_attr *data,
		       struct ubus_request_data *req)
{
	enum {
		NOTIFY_ATTR_COMMAND,
	...
	struct wpan_interface *vif = NULL;
	...
	case NOTIFY_CMD_SET_DATA:
		...
		*pdata = blob_memdup(cur);
		if (vif)
			wpan_interface_set_data(vif);
		break;
	case NOTIFY_CMD_PROCESS_ADD:
		return wpan_device_add_process(wdev, cur);
	case NOTIFY_CMD_PROCESS_KILL_ALL:
		return wpan_device_process_kill_all(wdev, cur, req);
	case NOTIFY_CMD_SET_RETRY:
		return wpan_device_set_retry(wdev, cur);
	default:
		return UBUS_STATUS_INVALID_ARGUMENT;
	}
